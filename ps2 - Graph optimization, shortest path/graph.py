...
class WeightedEdge(Edge):
    '''
    Represents a weighted edge in the dictionary. 
    Includes:
        src - string, source building
        dest - string, destination building
        total_distance - int, total distance between the source and destination buildings
        outdoor_distance - int, the distance required to travel outdoors
    '''
    ...
    def __str__(self):
        return self.src.get_name()+'->'+self.dest.get_name()+' '+str((self.total_distance,self.outdoor_distance))
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if node in self.edges:
             raise ValueError('Duplicate node in the graph')
        else:
             self.edges[node] = []
             self.nodes.add(node)
...
# print(type(e1.get_destination()))

# print(g)
# print(g.get_node('a'))
# print(str(g.has_node(g.get_node('a'))))
